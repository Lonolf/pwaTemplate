...
export const BackButton = ({ edit, setEdit, ...props }: { edit: boolean; setEdit: (edit: boolean) => void }) => {
  return (
    <Button
      onClick={() => {
        history.back()
...
export const CloseButton = ({ onClick, ...props }: { onClick: () => void }) => {
  return (
...
export const DeleteIconButton = ({ onClick }: any) => {
  return (
    <IconButton onClick={onClick}>
      <Delete sx={{ fill: theme => theme.palette.error.main }} />
    </IconButton>
  )
}
...
export const NewPatientButton = ({ onClick }: any) => {
  return (
    <Button variant='contained' color='secondary' onClick={onClick} sx={{ marginLeft: '10px' }}>
      <GroupAddIcon sx={{ fill: theme => '#fff' }} />
    </Button>
  )
}
